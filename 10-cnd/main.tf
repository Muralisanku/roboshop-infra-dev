resource "aws_cloudfront_distribution" "joinaiops" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.joinaiops.domain_name
        zone_id = aws_cloudfront_distribution.joinaiops.hosted_zone_id
      }
    }
  ]
}